...
void foobar(const char* str = __builtin_FUNCTION()){
    std::printf("called by %s\n", str); // trying to track where functions are called 
}
...
AbundanceMap global_abundance_map; // first declaration --> MG: test 19/11/2025
...
CompositionData parse_composition_data() {
    ...
    //printf("running parse_composition_data!!! TEST \n");
    ...
    **/
    ...
    // <-- NEW: cache globally so callers can use it without re-parsing
    global_abundance_map.comp_data = data;
    global_abundance_map.Zmix = data.Zsol; // moving to over here, its already calculated
...
double_map initial_composition_cno_cycle_core(const CompositionData &data, double X, double Z) {
    // Start from the standard composition
    double_map comp = update_initial_composition(data, X, Z);

	//printf("DEBUG start: C12=%.6e, N14=%.6e, O17=%.6e\n",
    //   comp["C12"], comp["N14"], comp["O17"]);

    // Access global abundance map for atomic weights
    AbundanceMap& abundance_map = global_abundance_map;

    // Ensure the isotopes exist before modifying
    if (comp.find("C12") != comp.end() && comp.find("N14") != comp.end() && comp.find("O17") != comp.end()) {
        // Add converted material from C12 and O17 into N14
        comp["N14"] += abundance_map.A_weights["N"] *
                      ((comp["C12"] / abundance_map.A_weights["C"]) +
                       (comp["O17"] / abundance_map.A_weights["O"]));

        // Zero out the source isotopes
        comp["C12"] = 0.0;
        comp["O17"] = 0.0;
    }

	//printf("DEBUG after conversion: N14=%.6e, C12=%.6e, O17=%.6e\n",
    //   comp["N14"], comp["C12"], comp["O17"]);

    // Recalculate total for bookkeeping
    double tot = comp.sum();
    comp["Ex"] = 1.0 - tot;
