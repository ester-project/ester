...
#include "ester-config.h"
          ...
          subroutine opa_opmesa(xchim, t, ro, kap, dkapt, dkapro, dkapx)
            ...
            enddo
              ...
              write(Xstr1_om,'(f5.3)')Xt(ix)
              write(Xstr2_om,'(f5.3)')Xt(ix+1)

              call init_highT_tables(Z,ierr)
              call init_lowT_tables(Z,ierr)
              ...
              if (use_mono) then
                ...
                ! Tables available for X = 0.0, 0.1, 0.2, 0.35, 0.5, 0.7, 0.8, 0.9, 0.95, 0.98
                if (logT .le. 4.5) then
            ...
            if (logT .ge. 3.75) then
              ...
              alpha = (X - X1)/(X2 - X1)
                ...
                logkap_highT1, dlnkap_rad_dlnT_highT1, dlnkap_rad_dlnRho_highT1, ix, ierr)

                call compute_kappa_fast(fk, logT, logRho, &
                logkap_highT2, dlnkap_rad_dlnT_highT2, dlnkap_rad_dlnRho_highT2, ix+1, ierr)
            ...
            if (logT .le. 4.5) then
                ...
                call compute_kappa_fast_lowT(&
                    logT, logR, &
                    lkap_ross_cell_lowT1, dlnkap_rad_dlnT_lowT1, dlnkap_rad_dlnR_lowT1, Xstr1, ierr)
            

                call compute_kappa_fast_lowT(&
                    logT, logR, &
                    lkap_ross_cell_lowT2, dlnkap_rad_dlnT_lowT2, dlnkap_rad_dlnR_lowT2, Xstr2, ierr)
          ...
          !write(*,*) 'ester_test', zbar, logRho, logT 
