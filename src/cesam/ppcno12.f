...
c**************************************************************************
	...
	SUBROUTINE ppcno12(t,ro,comp,dcomp,jac,deriv,fait,
...
c	H1, H2, He3, He4, Li7, Be7, C12, C13, N14, N15, O16, O17, Ex
c	Ex est l'élément fictif complément
c	aucun élément à l'équilibre

c	un premier appel à rq_reac initialise les
c	d'éléments chimiques pour lesquels les réac. nuc. sont tabulées
c	dans ppcno12 on ajoute Ex, soit nchim+1, puis,
...
c	 calcule l'abondance H1, H2, He3, He4, Li7, Be7, C13, C13, N14,
c	 N15, O16, O17, Ex
...
c Ex : élément fictif moyenne des éléments # Li + Be + CNO
	 ...
	 b1: DO i=5,nelem_ini		!a partir de Be+1=4+1
...
c élément fictif
	 ...
	 zi(nchim)=charge_ex	!charge de l'élément chimique reliquat
       i = nint(charge_ex)
	 nom_elem(nchim)=elem(i)//text	!elem. chim. rel.
...
c determination des abondances initiales, a(équation,élément)	 
...
c	 ENDDO
	 ...
	 IF(.not.inversible)THEN
	  PRINT*,'ppcno12, matrice du calcul des abond. non inversible'
	  PRINT*,'ARRET' ; STOP
	 ENDIF
...
c	 ab_min(13)=1.d-6	!Ex
	 
	 ab_min=ab_ini*fmin_abon ; ab_min(6)=1.d-14
...
1	 FORMAT('H1:',es10.3,', H2:',es10.3,', He3:',
	...
	4 'O16:',es10.3,', O17:',es10.3,', Ex:',es10.3)
	 WRITE(2,*)
	 WRITE(2,21)comp(5)/nbz,comp(6)/nbz,(comp(7)+comp(8))/nbz, !C
	1 (comp(9)+comp(10))/nbz,	!N
	2 (comp(11)+comp(12))/nbz,	!O
	3 comp(13)/nbz			!other
21	 FORMAT(1x,'rapports en nombre: Li/Z:',es10.3,', Be7/Z:',es10.3,
	1 ', C/Z:',es10.3,/,' N/Z:',es10.3,', O/Z:',es10.3,
	2 ', Ex/Z:',es10.3,/)
	 WRITE(2,*)'abondances négligeables:' ; WRITE(2,1)ab_min(1:nchim)
	 WRITE(2,*) ; WRITE(2,*)'aucun élément a l''équilibre'
	 ...
	 PRINT* ; WRITE(*,20)x0,y0,z0,z0/x0
	 PRINT* ; WRITE(*,1)ab_ini(1:nchim) ; PRINT*
	 WRITE(*,21)comp(5)/nbz,comp(6)/nbz,(comp(7)+comp(8))/nbz, !C
	1 (comp(9)+comp(10))/nbz,	!N
	2 (comp(11)+comp(12))/nbz,	!O
	3 comp(13)/nbz			    !other
	 PRINT*,'abondances négligeables:'
	 WRITE(*,1)ab_min(1:nchim) ; PRINT*
	 PRINT*,'aucun élément a l''équilibre'
	 PRINT* ; PRINT*,'on utilise une table' ; PRINT*
	 PRINT*,'évolution temporelle, test de precision sur H1 et He4'
	 PRINT*
...
c	 PRINT*,'réactions' ; WRITE(*,2000)r(1:nreac) ; PAUSE
...
c	 équations d'evolution
	...
	3 +r(14)*comp(12))*comp(1)+2.d0*r(3)*comp(3)**2		!H1
...
c	 PRINT*,'ppcno12, vérifications SUM dcomp*nucleo=0'
c	 WRITE(*,2000)DOT_PRODUCT(dcomp,anuc) ; PAUSE'vérif' 

	 dcomp(13)=-DOT_PRODUCT(dcomp,anuc)/anuc(13) 	!cons. des baryons	
...
c	3 +r(14)*comp(12))*comp(1)+2.d0*r(3)*comp(3)**2		!H1
	...
	6  +drx(14,i)*comp(12)+2.d0*drx(3,i)*comp(3)**2		!H1
	  ENDDO
	   	 
c équation dcomp(2)
c dcomp(2)=(r(1)*comp(1)-r(2)*comp(2))*comp(1)	!H2
	  jac(2,1)=2.*r(1)*comp(1)-r(2)*comp(2)		!d /H1
...
c équation dcomp(3)
c dcomp(3)=r(2)*comp(1)*comp(2)-(2.*r(3)*comp(3)+r(4)*comp(4))*comp(3)				!He3
	  jac(3,1)=r(2)*comp(2)				        !d /H1
	  jac(3,2)=r(2)*comp(1)				        !d /H2
	  jac(3,3)=-4.d0*r(3)*comp(3)-r(4)*comp(4)		!d /He3
	  jac(3,4)=-r(4)*comp(3)			        !d /He4
	 
	  DO i=1,nchim		!dependances dues à l'effet d'écran
	   jac(3,i)=jac(3,i)+drx(2,i)*comp(1)*comp(2)
	1  -(2.d0*drx(3,i)*comp(3)+drx(4,i)*comp(4))*comp(3)
	  ENDDO
...
c	2 +r(14)*comp(12))*comp(1)			!He4
	...
	1 +r(12)*comp(10)+r(14)*comp(12)		!d /H1
	  jac(4,3)=2.*r(3)*comp(3)-r(4)*comp(4)		!d /He3
	  jac(4,4)=-r(4)*comp(3)			!d /He4
	  jac(4,5)=r(5)*comp(1)*2.			!d /Li7
	  jac(4,6)=r(7)*comp(1)*2.			!d /Be7
	...
	3  +drx(12,i)*comp(10)+drx(14,i)*comp(12))*comp(1)
	  ENDDO
...
c dcomp(5)=-r(5)*comp(1)*comp(5)+r(6)*comp(6)*mue	!Li7
	  ...
	  DO i=1,nchim		!dépendances dues à l'effet d'écran
	   jac(5,i)=jac(5,i)
	1  -drx(5,i)*comp(1)*comp(5)+drx(6,i)*comp(6)*mue
	2  +r(6)*comp(6)*dmuex(i)
	  ENDDO
...
c dcomp(7)=(-r(8)*comp(7)+r(12)*comp(10))*comp(1)	!C12
	  ...
	  DO i=1,nchim		!dépendances dues à l'effet d'écran
	   jac(7,i)=jac(7,i)
	1  +(-drx(8,i)*comp(7)+drx(12,i)*comp(10))*comp(1)
	  ENDDO	 
...
c	  dcomp(13)=-dcomp(13)/anuc(13) 	!conservation des baryons
	  ...
	  DO j=1,13
	   DO i=1,12
	    jac(13,j)=jac(13,j)+anuc(i)*jac(i,j)
	   ENDDO
	  jac(13,j)=-jac(13,j)/anuc(13)
	  ENDDO

c unités de temps pour integration temporelle
	  jac=jac*secon6
...
c mue : nombre d'electrons / mole /g = 1/poids mol. moy. par e-
	...
	4  +(dqx(11,i)+dqx(12,i))*comp(10)
	5  +dqx(13,i)*comp(11)+dqx(14,i)*comp(12))*comp(1)
	  ENDDO
...
c production de neutrinos
	...
	CASE DEFAULT
	 PRINT*,'ppcno12, fait ne peut valoir que 1, 2, 3 ou 4'
	 PRINT*,'ERREUR fait a la valeur:',fait
	 PRINT*,'ARRET' ; PRINT* ; STOP
	 
	END SELECT

	END SUBROUTINE ppcno12
