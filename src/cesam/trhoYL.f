c*********************************************************************    
	...
	SUBROUTINE trhoYL(nom,tau,teff,logg,t,dtdtau,dtdteff,dtdg,rhom4,
	1 drhom4dteff,drhom4dg,ftau,dftau,d2ftau,kuredd)
...
c      LOGICAL, PARAMETER :: rad=.FALSE. !loi non purement radiative !enlevé par PM dans la V1
c     modif YLD      
c      LOGICAL, SAVE :: init=.TRUE.   !YLD
      LOGICAL, SAVE :: initkur=.TRUE.  !YLD
      LOGICAL :: kuredd     !YLD
      LOGICAL :: ok     
...
c	calculer t, rhom4 et leurs dérivées
	...
	IF(initkur)THEN                       !premier appel!YLD
	 initkur=.FALSE.              !YLD
...
c         initkur=.TRUE.        !YLD
         kuredd=.TRUE.       !YLD
c	 DEALLOCATE(tlrhom4(jmax,kmax),ty(imax,jmax,kmax))  !YLD
c	 STOP                                               !YLD
	 RETURN
