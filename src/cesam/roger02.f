...
c*******************************************************************
	...
	SUBROUTINE roger02(tau,teff,grav,t,dtsdtau,dtsdteff,dtsdg,
	1 ro_ext,dro_grav,dro_teff,f_tau,df_tau,d2f_tau)

c	routine private du module mod_atm
    
c	interface pour la loi T(tau) de R. cayrel avec [Fe/H]=+0.2
c	appel du fichier '~/.../fesh00.data'
...
1001	  FORMAT(/,'T(tau,teff,grav) law, roger02, no purely radiative',/,
...
1	  FORMAT(/,'loi T(tau,teff,grav), roger02, non purement radiative',
	...
	3 '3500K < Teff < 7000K, 1 < Log10(g) < 5, tau_min=1.d-4',/)
	 END SELECT
	 nom=TRIM(nom_chemin)//'fesh+02.data' ; rad=.FALSE.
	ENDIF
