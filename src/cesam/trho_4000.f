      SUBROUTINE trho_4000(nom,tau,teff,logg,t,dtdtau,dtdteff,dtdg,rhom4,
...
C     version modifiee pour tenir compte de la difference de pas
C       avant ou apres 4000 K ; stepteff doit etre nul comme indicateur
c     A cause de la recherche de tau* ou T(tau*)=Teff, comme on ne peut pas
...
C
         DO k = 1 , kmax
            teff_calc = teffinit + REAL(k-1) * stepteff                                                 ! A CHANGER
            DO j = 1 , jmax
...
C
               ...
               If ( ( pos == 0 ) .OR. ( teff_calc < 4749.0 ) ) Then          ! ne pas faire le test
                  If ( ABS(logg_calc-logg_lu) > 0.00001 ) Then
                     ...
                     STOP " TRHO PB 0 6 "
                  End If
                  If ( ABS(teff_calc-teff_lu) > 0.00001 ) Then                                    ! A CHANGER
                     Write(*,*) logg_calc, logg_lu, teff_calc, teff_lu
                     STOP " TRHO PB 0 7 "
                  End If
               End If
...
C
         ...
         teffmax = teffinit+(kmax-1)*stepteff                                 ! A CHANGER
...
C
c détermination du point pivot  : code change par B.Pichon avec des "-1" au lieu des "-2"
C
      logtau = LOG10(tau)
      ip = MIN( imax-1 , MAX ( NINT((logtau-logtauinit)/steplogtau)+1 , 2 ) )
      jp = MIN( jmax-1 , MAX ( NINT((logg_calc-logginit)/steplogg)+1 , 2 ) )
      kp = MIN( kmax-1 , MAX ( NINT((teff_calc-teffinit)/stepteff)+1 , 2 ) )                                 ! A CHANGER
c
c fin calcul pivot
c
...
c
      ...
      w=(teff_calc-(teffinit+stepteff*(kp-1)))/stepteff                                 ! A CHANGER
c     PRINT*, ' coucou3 ',' u= ',u, ' v= ',v, ' w= ',w
      CALL quadr2(tlrhom4,v,w,jp,kp,lrhom4,yv,yw)
      rhom4=10.d0**lrhom4
      g=10.d0**logg_calc
      dlrhom4dteff=(yw/stepteff)                                 ! A CHANGER
...
c
      t = teff_calc * y                                 ! A CHANGER
c     PRINT*,' t = ', t
      dtdteff = y + teff_calc * yw / stepteff                                 ! A CHANGER
