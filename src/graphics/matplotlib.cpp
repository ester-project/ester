...
void plt::clim(const double& z0, const double& z1) {
    ...
    PyList_SetItem(list, 0, PyFloat_FromDouble(z0));
    PyList_SetItem(list, 1, PyFloat_FromDouble(z1));
...
void plt::xlabel(const std::string& label) {
#if ENABLE_PLT
    if (noplot) return;
...
void plt::ylabel(const std::string& label) {
#if ENABLE_PLT
    if (noplot) return;
