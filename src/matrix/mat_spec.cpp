...
template<>
Matrix<float> Matrix<float>::operator,(const Matrix<float> &a) const {
	...
	Matrix<float> res(nf,a.nc);

	cblas_sgemm(CblasColMajor,CblasNoTrans,CblasNoTrans,nf,a.nc,nc,1,p,nf,a.p,a.nf,0,res.p,res.nf);

	return res;
}

template<>
Matrix<double> Matrix<double>::solve(Matrix<double> b) const {
...
template<>
Matrix<float> Matrix<float>::solve(Matrix<float> b) const {

	int ipiv[nf],info;
	char trans='N';
	Matrix<float> lu(*this);
	...
	sgetrf_(&lu.nf,&lu.nc,lu.p,&lu.nf,ipiv,&info);
	if(info>0) {
		printf("Matrix is singular (%d)\n",info);
		exit(1);
	}
	sgetrs_(&trans,&lu.nf,&b.nc,lu.p,&lu.nf,ipiv,b.p,&b.nf,&info);
...
template<>
Matrix<double> Matrix<double>::inv() const {
...
template<>
Matrix<float> Matrix<float>::inv() const {
	...
	sgetrf_(&res.nf,&res.nc,res.p,&res.nf,ipiv,&info);
	if(info>0) {
		printf("Matrix is singular (%d)\n",info);
		exit(1);
	}
	sgetri_(&res.nf,res.p,&res.nf,ipiv,&w,&lwork,&info);
	lwork=round(w);

	work=new float[lwork];
	
	sgetri_(&res.nf,res.p,&res.nf,ipiv,work,&lwork,&info);
