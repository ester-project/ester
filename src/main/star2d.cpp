...
int main(int argc, char *argv[]) {
    ...
    while (!last_it) {
        ...
        tt(nit - 1) = t.value();
        error(nit - 1) = err;
        last_it = (err < config.tol && nit >= config.minit)
            || nit >= config.maxit;
        if (killed)
            last_it = 1;
        if (config.verbose) {
    ...
    if (config.verbose) {
        ...
        printf("Teff(p)=%.2f K      Teff(e)=%.2f K\n",
                A.map.leg.eval_00(A.Teff(), 0) (0), A.map.leg.eval_00(A.Teff(),
                    PI /
                    2) (0));
        printf("log(geff)(p)=%.2f       log(geff)(e)=%.2f\n",
                A.map.leg.eval_00(log10(A.gsup()), 0) (0),
                A.map.leg.eval_00(log10(A.gsup()), PI / 2) (0));
        double wp = A.map.leg.eval_00(A.w.row(-1), 0) (0) * A.units.Omega;
        double we =
            A.map.leg.eval_00(A.w.row(-1), PI / 2) (0) * A.units.Omega;
        double Pp = 2 * PI / wp / 3600. / 24.;
        double Pe = 2 * PI / we / 3600. / 24.;
        printf
            ("P_rot(p)=%.3f days     P_rot(e)=%.3f days      v_eq=%.2f km/s\n",
             Pp, Pe, we * Re / 1e5);
        ...
        if (A.conv)
            printf("R. conv. core (p)=%3.3f Rsun\n",
