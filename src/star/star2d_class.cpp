...
star2d::star2d() : nr(map.gl.N), nth(map.leg.npts), nex(map.ex.gl.N),
    ...
    Dt2(map.Dt2), zex(map.ex.z), Dex(map.ex.D), rex(map.ex.r), D(map.D) {
...
write_attr(H5::Group grp, const char *name, H5::DataType type,
        const T ptr, int n = 1) {
...
void star2d::hdf5_write(const char *filename) const {
    ...
    write_attr(star, "domain_type", integer, domain_type.data(),
            map.ndomains);
    ...
    H5::StrType strtype;
    strtype =
        H5::StrType(H5::PredType::C_S1, strlen(version.name.c_str()) + 1);
...
read_attr(H5::Group grp, const char *name, T mem) {
    try {
...
int star2d::hdf5_read(const char *input_file, int dim) {
    ...
    if (read_attr(star, "min_core_size", &min_core_size)) {
        ester_warn("Could not read 'min_core_size' from file `%s'",
                input_file);
        min_core_size = 0.03;
    }
...
int star2d::read(const char *input_file, int dim) {
    ...
    if (version.svn) {
        if (asprintf(&buf, "%d.%d rev %d",
                    version.major, version.minor, version.rev) == -1) {
    ...
    if (fp.read("atm.name", atm.name))
        strcpy(atm.name, "simple");
    if (fp.read("Omega", &Omega))
        Omega = 0;
    if (fp.read("Omega_bk", &Omega_bk))
        Omega_bk = 0;
    if (fp.read("Ekman", &Ekman))
        Ekman = 0;
    if (fp.read("core_convec", &core_convec))
        core_convec = 1;
    if (fp.read("env_convec", &env_convec))
        env_convec = 0;
    if (fp.read("min_core_size", &min_core_size))
        min_core_size = 0.03;
    if (fp.read("stratified_comp", &stratified_comp))
        stratified_comp = 0;
...
int star2d::read_old(const char *input_file) {
    ...
    if (mode == 'b') {
        ...
        if (fread(map.gl.npts, sizeof(int), ndom, fp) != (size_t) ndom)
            ester_warn("read failed");
        if (fread(map.gl.xif, sizeof(double), ndom + 1, fp) !=
                (size_t) ndom + 1)
            ester_warn("read failed");
...
int star2d::init(const char *input_file, const char *param_file,
        int argc, char *argv[]) {
    DEBUG_FUNCNAME;
    cmdline_parser cmd;
    file_parser fp;
                ...
                if (*param_file) {
                    if (fp.open(param_file)) {
                        while ((k = fp.get(arg, val))) {
                            if (!strcmp(arg, "nth") && val)
                                nt = atoi(val);
                            if (!strcmp(arg, "nex") && val)
                                next = atoi(val);
                        }
                        fp.close();
                    }
...
void star2d::init1d(const star1d &A, int npts_th, int npts_ex) {
    ...
    *this = A;
    ...
    if (fp.open(default_params)) {
        while ((k = fp.get(arg, val))) {
            if (!strcmp(arg, "nth") && val && npts_th == -1)
                npts_th = atoi(val);
            if (!strcmp(arg, "nex") && val && npts_ex == -1)
                npts_ex = atoi(val);
            if (!strcmp(arg, "Omega_bk") && val)
                Omega_bk = atof(val);
            if (!strcmp(arg, "Ekman") && val)
                Ekman = atof(val);
        }
        fp.close();
    }
    ...
    if (!strcmp(arg, "ndomains")) {
        ...
        *change_grid = *change_grid | 1;
        ...
        while (tok != NULL) {
            ...
            params.npts = (int *) realloc(params.npts,
                    (i+1)*sizeof(params.npts[0]));
            if (params.npts == NULL) {
                ester_err("out of memory");
                exit(EXIT_FAILURE);
            }
            params.npts[i] = atoi(tok);
            tok = strtok(NULL, ",");
            i++;
        }
        params.nnpts = i;
        ...
        *change_grid = *change_grid | 4;
        ...
        ester_err("Param. conv is no longer modifiable %s",
                "Disable core convection with core_convec 0");
        return 1;
    }
