...
void star2d::opacity() {
	...
	error=opa_calc(comp.X(),comp.Z(),Tc*T,rhoc*rho,opa);
...
void star2d::eq_state() {
	...
	matrix rhoc_m(1,1);
	eos_calc(comp.X()(0,0)*ones(1,1),comp.Z()(0,0)*ones(1,1),ones(1,1)*Tc,ones(1,1)*pc,rhoc_m,eos);
	//eos_calc(comp.X()(0,0)*ones(1,1),Z0,ones(1,1)*Tc,ones(1,1)*pc,rhoc_m,eos);
	rhoc=rhoc_m(0);

	error=eos_calc(comp.X(),comp.Z(),T*Tc,p*pc,rho,eos);
...
//	rhoc=rho(0);
	...
	if(error) ester_err("Error in eos_calc\n");
...
void star2d::atmosphere() {
	...
	//error=atm_calc(comp.X(),comp.Z(),gsup(),Teff(),eos.name,opa.name,atm);
