...
void star1d::atm_simple() {
	...
	ps=2./3.*surff*(D.row(nr-1),phi)(0)/opa.k(nr-1)/rhoc/R;
	Ts=0.5*(surff*p(nr-1)/ps+1)/Tc/Tc/Tc/R*Frad(nr-1)*Lambda/SIG_SB;
	Ts=pow(Ts,1./4.);
...
void star1d::solve_atm_simple(solver *op) {
	...
	q=3*ones(1,1);
	op->bc_top1_add_d(ndomains-1,"Ts","Tc",q);
	q=-surff*p(-1)/(surff*p(-1)+ps)*ones(1,1);
	op->bc_top1_add_d(ndomains-1,"Ts","p",q);
