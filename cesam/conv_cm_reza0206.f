...
c****************************************************************
	...
	SUBROUTINE conv_cm_reza0206(krad,grav,delta,cp,ro,hp,gradrad,gradad,der,
...
c 
c       Updated version (10.06.02) by Reza Samadi, LESIA , Observatoire de Paris 
c       in the original paper (CM91) the  Kolmogorov constant K0=1.5 is assumed
c	according to the correct value  K0=5/3 assumed in Heiter et al (A&A, 2002)
c       phi is multiply in thiw new version by (5/3 / 1.5)^3 = 1.3717421
c      
c entree :
...
c	Newton-Raphson "d" means dérivative/ grad	
	...
	do while(.NOT.conv)
	 ...
	 IF(iter > 30)THEN
	  ...
	  PRINT*,'données : krad,grav,cp,ro,hp,gradrad,gradad'	  
	 ...
	 phi=a1*sig**m*(a2sn-1.d0)**p *  1.3717421 ! 1.3717421=(5/3 / 1.5)^3  [RS 10.06.02]
...
c	 phi=a1*sig**m*(a2sn-1.d0)**p
	...
	END SUBROUTINE conv_cm_reza0206
