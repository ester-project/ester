...
c	contributions d'Yveline Lebreton
...
c mesure de sécurité : le fichier modif_mix ne peut être pris en compte que si
c modif_chim = .TRUE.
...
c définition des rapports isotopiques, recherche d'un fichier de rap.iso.
...
c mesure de sécurité : le fichier rap_iso ne peut être pris en compte que si
c modif_chim = .TRUE.
	 IF(.NOT.modif_chim)THEN
	  SELECT CASE(langue)
	  CASE('english')	
