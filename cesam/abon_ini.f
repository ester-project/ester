...
c initialisation des abondances, des rapports isotopiques,
c	des charges des éléments. Les abondances initiales des isotopes
c	utilisés dans le réseau nucléaire seront déduites de ces valeurs
c	initiales suivant la chaine de réactions utilisées et les isotopes
c	seulement diffusés.
c	source : ftp:://www-phys.llnl.gov/pub/opal

c pour ajouter un élément:
c	dans le module mod_nuc augmenter le PARAMETER nelem_ini d'une
c	unité, Ex: nelem_ini=28

c	Auteur: P.Morel, Département Cassiopée, O.C.A.
c	Conseils de F.Thévenin, Département Cassiopée, O.C.A. 
c	CESAM2k
...
c	ln_Tli : température au dessus de laquelle il n'y a pas de lithium
c	inital pour le modèle de ZAMS Li initial

	ln_Tli=LOG(3.d6) ; li_ini=1.d-15

c charges et masses (OPAL) des éléments
	c=(/ (i, i=1,nelem_ini) /)
...
c abondances initiales
	SELECT CASE(nom_abon)
	CASE('meteorites_ag')	!la normalisation à H=1.d12 est effectuée
	 SELECT CASE(langue)	!plus loin
	 CASE('english')	
...
1	  FORMAT('Mixture des météorites de Anders & Grevesse 89')
	...
	CASE('meteorites_gs')	!la normalisation à H=1.d12 est effectuée
	 SELECT CASE(langue)	!plus loin
	 CASE('english')	
...
26	  FORMAT('Mixture des météorites de Grevesse & Sauval 98')
	...
	CASE('meteorites_gs_h2')	!la normalisation à H=1.d12 est effectuée
	 SELECT CASE(langue)	!plus loin
	 CASE('english')	
	  WRITE(*,1026) ; WRITE(2,1026)
	 CASE DEFAULT
...
17	  FORMAT('Mixture solaire de Grevesse & Sauval 98')
 ...
 1019	  FORMAT('Solar mixture of AGS 05')	
	 CASE DEFAULT			
	  WRITE(*,19) ; WRITE(2,19)
 19	  FORMAT('Mixture solaire de AGS 05')
	 END SELECT
...
1011	  FORMAT('STOP, the initial mixture : ',a,/,'is unknown',/,
	...
	2 'solaire_gn, solaire_ags05, enhan_cha, enhan_w, enhan_al, mixture')	  
	 CASE DEFAULT	  
...
c pour sécurité, on ne recherche ce fichier que si modif_chim

	I1: IF(modif_chim)THEN	
...
c si garde_xish = .TRUE. les rapports Xi/H seront ceux de la
c	mixture, l'abondance d'hélium sera Y0 lue dans le fichier *.don.
c	X0 et Z0 seront déduits de la mixture
c si garde_xish = .FALSE. les rapports Xi/H seront modifiés
c	les rapports Xi/Z seront ceux de la
c	mixture, X0, Y0, Z0 seront ceux du fichier *.don
c	(exemple : la calibration solaire en Z/X)

	IF(garde_xish)THEN	
c ajustement de ab(2) pour obtenir Y=Y0		
	 nisnh=ab*m*1.d-12
	 ab(2)=y0/(1.d0-y0)/m(2)*(m(1)+SUM(nisnh(3:28)))*1.d12	 
c	 WRITE(*,2000)ab(2) ; PAUSE'ab(2)'

c détermination de X0, Y0, Z0
	 sniai=SUM(ab*m)
...
14	  FORMAT(/,'X0:',es10.3,', Z0:',es10.3,', Z0/X0:',es10.3,
	...
	ELSE
	 ...
	 CASE('english')
	  WRITE(*,1018)x0,y0,z0
	  WRITE(2,1018)x0,y0,z0
1018	  FORMAT(/,'One uses the input data X0:',es10.3,', Y0:',es10.3,
	1', Z0:',es10.3,/,'and the ratios metal/Z of the mixture')	
	 CASE DEFAULT	 
	  WRITE(*,18)x0,y0,z0
	  WRITE(2,18)x0,y0,z0
18	  FORMAT(/,'On utilise les données X0:',es10.3,', Y0:',es10.3,
	1', Z0:',es10.3,/,'et les rapports métal/Z de la mixture')
	 END SELECT
	ENDIF
...
c abondances(en nombre) des métaux dans Z
	abon_rela=ab/SUM(ab(3:nelem_ini))
	SELECT CASE(langue)	  
	CASE('english')	
	 WRITE(*,1039) ; WRITE(2,1039)
1039	 FORMAT(/,'relative abondunces(by number) of metals within Z')
	CASE DEFAULT	 
	 WRITE(*,39) ; WRITE(2,39)
39	 FORMAT(/,'abondances(en nombre) des métaux dans Z')	
	END SELECT	 
	WRITE(*,2000)abon_rela(3:nelem_ini)
	WRITE(2,2000)abon_rela(3:nelem_ini)

c produit abondance * masse pour calcul des abondances relatives en masse
c	et écriture des abondances des métaux dans Z	
	abon_m=ab*m
	abon_relam=abon_m/SUM(abon_m(3:nelem_ini))	
	SELECT CASE(langue)	  
	CASE('english')	
	 WRITE(*,1025) ; WRITE(2,1025)
1025	 FORMAT(/,'relative abondunces(by mass) of metals within Z')
	CASE DEFAULT	 
	 WRITE(*,25) ; WRITE(2,25)
...
c définition des rapports isotopiques, recherche dun fichier de rap.iso.
...
c les rapports isotopiques ne peuvent être différents de ceux d'Anders
c & Grevesse que si modif_chim	
	ok=ok .AND. modif_chim	  
	IF(ok)THEN
	 OPEN(unit=3,form='formatted',status='old',delim='apostrophe',
	1 file=TRIM(chain))
...
6	  FORMAT('Rapports isotopiques suivant Anders & Grevesse',/,
	1 'sauf He3/He4 de Gautier & Morel 97')
	 END SELECT
	 IF (nom_abon == 'meteorites_gs_h2') THEN
	  h2sh1=1.5d-5		 !Linsky
	 ELSE
...
22	FORMAT('Li6/Li7=',es10.3,', Be7/Be9=',es10.3, ', C13/C12=',es10.3)
	WRITE(*,23)n15sn14,o17so16,ne22sne20
	WRITE(2,23)n15sn14,o17so16,ne22sne20	
23	FORMAT('N15/N14=',es10.3,', O16/O17=',es10.3,', Ne22/Ne20=',es10.3)
