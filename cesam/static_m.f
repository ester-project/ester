...
c----------------------------------------------------------------
	...
	USE mod_donnees, ONLY : alpha, cpturb, ctel, ctem, ctep,
	1 cter, ctet, dtmin, d_grav, g, Ipg, Krot, kipp, langue, lsol, mdot, msol,
	2 m_ch, m_ptm, m_rot, m_tds, nchim, ne, nrot, pi, pturb, ro_test,
	3 rsol, secon6, t_inf
	...
	3 n_qs, n_rot, n_qs_t, n_tds_t, old_ptm, qt_t, q_t, rota, r2_t,
	...
	1 dxchiml, dxchim_t, xchim, xchim_t	
	...
	REAL (kind=dp), SAVE ::	cte1, cte6, cte11, cte13, cte14, cte14t,
	1 cte15, cte21, cte22, cte23, cte24, cte25, dmdl, dmdr, dpdl, dpdr,
	2 dptdl, dptdr, dtdl, dtdr, dt_tds, l13, mext, m13,
	3 pext, ray, text, unpdd
	...
	7 dhpr, dhpm,
	8 dkapp, dkapt, dkapx, dlpp, dlppdp, dlppdpt, dpn, dpsitl,
	9 dpsitm, dpsitp, dpsitr, dpsitt, dp_tm=0.d0, drom, drop, drop_t,
	1 drot, drot_t=0.d0, drox, drox_t=0.d0, dro_tm=0.d0, dstor,
	...
	2 gamma1, grad, grad0 ,gradrad, gravs2, hp, kap,
	3 ln, mext0, mk, mn, pext0, pgn, prn, psist, psist0, ptext, ptext0,
	4 p_t=0.d0, p_t0, ro, ro0, ro_t=0.d0, ro_t0, stor, stor0,
	5 tdst=0, tds0, teff, teta, teta0, text0, trn, t_t=0.d0, t_t0, u,
	6 u_t=0.d0, u_t0, u0, x00, v, w, dtn1, dpn1 

	INTEGER, SAVE :: l
	INTEGER :: i, id, j, jd
	
	LOGICAL, SAVE :: deriv=.FALSE., init=.TRUE., der
	LOGICAL :: radiatif
...
c	WRITE(*,2000)y(1:ne,1)	
	...
	IF(init)THEN
	 ...
	 dt_tds=100.d0*dtmin	!dt < dt_tds on utilise la tabulation TdS
...
c accélération centrifuge	 
	acc_c=rsol*ray*w**2 ; reprend=acc_c > gravs2
	IF(reprend)THEN
	 SELECT CASE(langue)
	 CASE('english')
	   WRITE(*,1012)acc_c,gravs2,w ; WRITE(2,1012)acc_c,gravs2,w
1012	   FORMAT('STOP, in static_m, the centrifugal acceleration =',
	1    es10.3,' > 90% gravity = ',es10.3,/,'angular velicity =',
	2    es10.3)
	  CASE DEFAULT
	   WRITE(*,12)acc_c,gravs2,w ; WRITE(2,12)acc_c,gravs2,w
12	   FORMAT('ARRET, dans static_m, accélération centifuge =',
...
c	 la physique est violée... mais si ca passe les erreurs d'arrondi
c	 disparaissent, parfois, au cours des iterations
c	 pour eviter cette disposition supprimer les "c" devant reprend et RETURN

	 IF(y(3,0)*y(5,0)*y(4,0) < 0.d0)THEN
	  WRITE(*,"('static_m, r2, l23, ou m23 < 0, on tente de CV, cx=',i4)")cx
c	   WRITE(*,2000)xcoll(cx),y(3,0),y(5,0),y(4,0)
c	   PRINT*,y(1:ne,0)
c	   PAUSE'erreur, static_m, m, r ou l < 0'
c	   reprend=.TRUE. ;  RETURN
	 ENDIF
...
c	 composition chimique au temps t+dt
	...
	1 knotc,.TRUE.,MIN(ay5,mc(n_ch)),l,xchim,dxchim)
	 ...
	 xchim=ABS(xchim)
	 dxchiml=dxchim*2.d0/3.d0/m13	!dérivée / m pour Ledoux
	
c	 PRINT*,'cx,nchim/prn,pgn,trn,mn,ray / xchim / y  /dy',cx,nchim
c	 WRITE(*,2000)prn,pgn,trn,mn,ray,xchim(1),dxchiml(1)
...
c	 PRINT*,' '
	 ...
	 ENDIF
	 CALL thermo(prn,pgn,trn,mn,ln,ray,dlpp,xchim,dxchiml,
...
c------------------------------------------------------------------------------
	   mk=ay5*(1.d0-1.d6*mdot/mstar*dt)**(2.d0/3.d0) ! effects of accr!
	   CALL bsp1dn(1,tds_t,x_tds_t,xt_tds_t,n_tds_t,m_tds,
	1    knot_tds_t,.TRUE.,MIN(mk,x_tds_t(n_tds_t)),l,ftds,dftds)
c------------------------------------------------------------------------------
c	   CALL bsp1dn(1,tds_t,x_tds_t,xt_tds_t,n_tds_t,m_tds,
c	1    knot_tds_t,.TRUE.,MIN(ay5,x_tds_t(n_tds_t)),l,ftds,dftds)
...
c------------------------------------------------------------------------------
	   mk=fm(1)*(1.d0-1.d6*mdot/mstar*dt)**(2.d0/3.d0) ! effects of accr!
c------------------------------------------------------------------------------
	   CALL inter('m23',bp_t,q_t,qt_t,n_qs_t,knot_t,mk,fqs,dfqs,r2_t,
	1    m23_t)
	   ...
	   IF(kipp)THEN	!approximation de Kippenhahan
	    dpn1=-2.d0/3.d0*ay5*1.d6*mdot/mstar*dp_tm*dt
	    dtn1=-2.d0/3.d0*ay5*1.d6*mdot/mstar*dt_tm*dt	    
	    dpn=dpn1+pgn-p_t ; dtn=dtn1+trn-t_t
...
c	     duv=MAX(ABS(dpn/pgn),ABS(dtn/trn))
	    ...
	    IF(reprend)THEN		!TdS varie trop
	     WRITE(*,7)mn
7	     FORMAT('Approx. Kipp. trop de var. de TdS en m=',es10.3)
...
c	     duv=MAX(ABS(u-u_t)/u,ABS(ro-ro_t)/ro)
	    ...
	    IF(reprend)THEN		!TdS varie trop
	     WRITE(*,6)mn
6	     FORMAT('trop de var. de TdS en m=',es10.3)
...
c contribution du Tds/dt	
	 IF(dt /= 0.d0)THEN
	  ...
	  IF(compt == 0)THEN
	   dtdsm= dtdsm+tdst/2.d0/ay5+dy5*dtdst_m	!dérivée/m**2/3
	   ...
	   ae(4,4,0)=ae(4,4,0)+psist*dtdsl		!dérivée/l**2/3
...
c ---- new
c------------------------------------------------------------------------------
c	    dtdsm= dtdsm+dy5*(dcpm*dtn-cp*dt_tm*(1.d0+2.d0/3.d0*1.d6*mdot/mstar*dt) 	!dérivée/m**2/3
c	1   -delta/ro*((deltam/delta-drom/ro)*dpn-dp_tm*(1.d0+2.d0/3.d0*1.d6*mdot/mstar*dt)))/dt
c------------------------------------------------------------------------------
c ---- old
...
c	 deriv= cx >= 299 .AND. cx <= 303
	...
	1 fac(ip)*(yd(4,0)*ctel+yd(1,0)*ctep+yd(5,0)*ctem+yd(3,0)*cter)
	 ...
	 PRINT*,'psist,teta,dpsitp,dpsitt,dpsitr,dpsitm,dpsist'
	 WRITE(*,2000)psist,teta,dpsitp,dpsitt,dpsitr,dpsitm,1./teta
...
c	   PAUSE'avant bsp1dn'
	...
	1    knotc,.TRUE.,MIN(ay5,mc(n_ch)),l,xchim,dxchim)
	   ...
	   xchim=ABS(xchim)
	   dxchiml=dxchim*2.d0/3.d0/m13		!dX/dm pour Ledoux
	...
	1     ay5,l,frot,dfrot)
	   ...
	   v=SQRT(ay5/ay3)/prn*w**2 ; dyv=cte25*fac(ip)*v	!rotation
	   
	   teta=fac(ip)*((ay5/ay3)**2/prn*(cte21+cte22*grad)
...
c	 li=7 : limite sur rho au raccord
c	 li=Ipg : limite sur Pgaz au raccord avec Pturb

	 deriv=.FALSE. !; deriv=.TRUE.	   
	 IF(deriv)THEN
...
c	   PAUSE'au centre'
	 ...
	 CASE(4)		!Ptot au raccord
	  CALL bsp1dn(nchim,chim,mc,mct,n_ch,m_ch,
	1   knotc,.TRUE.,MIN(y(5,0),mc(n_ch)),l,xchim,dxchim)
	...
	1   mext,dmdl,dmdr,pext,dpdl,dpdr,teff)
	  ...
	  ae(1,3,0)=-dptdr/ptext/2.d0/ray	!dérivée/r**2
	  ae(1,4,0)=-dptdl/ptext*3.d0/2.d0*l13	!dérivée/l**2/3
...
c	    PRINT*,LOG(ptext),yd(1,0),yd(1,0)-LOG(ptext),be(1)
	 ...
	 CASE(5)	  !T au raccord
	  ...
	  ae(1,3,0)=-dtdr/text/2.d0/ray		!dérivée/r**2
	  ae(1,4,0)=-dtdl/text*3.d0/2.d0*l13	!dérivée/l**2/3
...
c	   WRITE(*,2000)y(2,0),LOG(text),be(1)
	...
	1   mext,dmdl,dmdr,pext,dpdl,dpdr,teff)
	 ...
	 CASE(6)	!condition sur M au raccord
	  be(1)=y(5,0)-mext**(2.d0/3.d0)	!en q=n_qs, m=mext(r,l)
	  ae(1,3,0)=-dmdr/3.d0/ray/m13		!dérivée/r**2
	  ae(1,4,0)=-dmdl*l13/m13		!dérivée/l**2/3
	  ae(1,5,0)=1.d0			!dérivée/m**2/3
c	  WRITE(*,2000)y(5,0)
	  IF(deriv)THEN 	!test de dérivation
	   PRINT*,'limite mext',li
	   WRITE(*,2000)y(5,0),mext**(2.d0/3.d0)
	   WRITE(*,2000)y(1:ne,0)
	   PRINT*,'limite y(5),mext',li
	   PRINT*,'y(5,0),mext,be(1)'
	   WRITE(*,2000)y(5,0),mext,be(1)
	   ptext0=ptext ; text0=text ; mext0=mext ; pext0=pext
	   DO i=3,4
	    stor0=yd(i,0) ; stor=stor0*unpdd
	    IF(stor == 0.d0)stor=dd
	...
	1     mext,dmdl,dmdr,pext,dpdl,dpdr,teff)
	 ...
	 CASE(7)	!condition sur rho au raccord 
	 CALL bsp1dn(nchim,chim,mc,mct,n_ch,m_ch,
	1  knotc,.TRUE.,mc(n_ch),l,xchim,dxchim)
	 IF(no_croiss)PRINT*,'Pb. at 12 in static_m'
	 xchim=ABS(xchim) ;  CALL chim_gram(xchim,dxchim)	
	 CALL etat(EXP(y(1,0)),EXP(y(2,0)),xchim,.FALSE.,
	1  ro,drop,drot,drox,u,dup,dut,dux,
	2  delta,deltap,deltat,deltax,cp,dcpp,dcpt,dcpx,
	3  gradad,dgradadp,dgradadt,dgradadx,alfa,beta,gamma1)
	 be(1)=y(7,0)-LOG(ro)
	 ...
	 ae(1,2,0)=delta	 
	 ae(1,7,0)=1.d0	 
c	 PRINT*,y(1,0),y(2,0),y(7,0),LOG(ro)
c	 WRITE(*,2000)alfa,delta,be(1),ro
 	  	  
	 CASE(8)	!condition sur Pgaz au raccord
	  be(1)=y(Ipg,0)-LOG(pext)				!en q=n
	  ae(1,3,0)=-dpdr/pext/2.d0/ray		!dérivée/r**2
	  ae(1,4,0)=-dpdl/pext*3.d0/2.d0*l13	!dérivée/l**2/3
	  ae(1,Ipg,0)=1.d0			!dérivée/ln Pgaz
	  IF(deriv)THEN 	!test de dérivation
	   PRINT*,'limite pext, uint',li
	   WRITE(*,2000)y(Ipg,0),pext ; WRITE(*,2000)y(1:ne,0)
	   PRINT*,'limite y(Ipg)',li ; PRINT*,'y(Ipg,0),pext,be(1)'
	   WRITE(*,2000)y(Ipg,0),pext,be(1)
	   ptext0=ptext ; text0=text ; mext0=mext ; pext0=pext
	   DO i=3,4
	    stor0=yd(i,0) ; stor=stor0*unpdd
	    IF(stor == 0.d0)stor=dd
