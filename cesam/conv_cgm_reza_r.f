...
c****************************************************************
	...
	SUBROUTINE conv_cgm_reza_r(r,krad,grav,delta,cp,ro,hp,gradrad,
...
c---------------------------------------------------------------------------
	...
	USE mod_donnees, ONLY: alpha, rsol
	USE mod_kind	
	USE mod_variables, ONLY: id_conv, if_conv, r_zc_conv, hp_zc_conv
	
	IMPLICIT NONE
		
	REAL (kind=dp), INTENT(in) :: r, krad, grav, delta, cp, ro, hp,
	1 gradrad, gradad		
	...
	REAL (kind=dp) :: l, l0, ki, a, corr, b, sig, dsig,
...
1	 FORMAT(/,'gradient dans zone convective calculé selon',/,
	...
	B1: DO i=id_conv,if_conv,2
	 IF(r >= r_zc_conv(i) .AND. r <= r_zc_conv(i+1))THEN
	  zup=(r_zc_conv(i+1)-r)*rsol + alpha*MIN(hp_zc_conv(i+1),r_zc_conv(i+1)*rsol)
	  zlow=(r-r_zc_conv(i))*rsol + alpha*MIN(hp_zc_conv(i),r_zc_conv(i)*rsol)
	  l=zup*zlow/(zup+zlow)
c	  PRINT*,i
c	  WRITE(*,2000)r_zc_conv(i),r,r_zc_conv(i+1)
c	  PRINT*,hp_zc_conv(i),l0,hp_zc_conv(i+1)
c	  PRINT*,r
c	  PRINT*,l0,hp,l0/hp
	  EXIT B1
	 ENDIF
	...
	IF(l < 0.d0) l=4.d0*alpha*hp
c	print*,'________________________________________________________________________'
c	print*,'  r-r_zc_conv(i)          r                       r_zc_conv(i+1)-r'
c	print*,r-r_zc_conv(i),r,r_zc_conv(i+1)-r
c	print*,'  l/hp                    r_zc_conv(i)            r_zc_conv(i+1)'
c	print*,l/hp,r_zc_conv(i),r_zc_conv(i+1)
c	print*,'  hp_low                  hp_up                   hp'
c	print*,hp_zc_conv(i)/rsol,hp_zc_conv(i+1)/rsol,hp/rsol

c	print*,r,l/hp

c	l=alpha*hp		!longueur de mélange
...
c	Newton-Raphson "d" signifie derivee/ grad	
	...
	DO WHILE(.NOT.conv)
	 iter=iter+1
	 IF(iter > 30)THEN
	  PRINT*,'pas de convergence dans conv_cgm_reza_r'
	  ...
	  print*,r_zc_conv(i),r,r_zc_conv(i+1)
	  PRINT*,'l/hp = ',l/hp ; STOP
	 ENDIF
...
c	 Phi  (Eq. 17 H02) :
	...
	END SUBROUTINE conv_cgm_reza_r
