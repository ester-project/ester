...
c	et de l'effet d'écran, on utilise les tables calculées par
c	tabul_nuc
...
c--------------------------------------------------------------------
	...
	REAL (kind=dp), ALLOCATABLE, SAVE, DIMENSION(:,:) :: decrx,
	1 dzetax, taux_reac				
	REAL (kind=dp), ALLOCATABLE, SAVE, DIMENSION(:) :: ar, datx, dbidx,
	1 dc1x, decro, decrt, dfx, df12x, dkx, dnex, dr, dzstarx, ecran,
	2 lambda, mz1z2, q0, temp, ttemp, tx, z1z2
	...
	INTEGER, SAVE :: i3al, knot_temp, l=1, m_temp, n_temp	
	INTEGER :: i, j
	
	CHARACTER (len=20), ALLOCATABLE, DIMENSION(:) :: nom_reac
...
c	 tabulation des réactions nucléaires
	 ...
	 CALL tabul_nuc(ar,i3al,knot_temp,m_temp,nom_reac,
	1 n_temp,q0,taux_reac,temp,ttemp)	 
...
c	 WRITE(*,2000)zstar,mue,ne,c1,f12,at,k
	 ...
	 DO i=1,nreac	  
c	  WRITE(*,5)nom_reac(i)
	  IF(izz(i,1) > 0)THEN	!pour passer les captures e-
	   DO j=1,2
