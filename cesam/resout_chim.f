...
c----------------------------------------------------------------
	...
	USE mod_donnees, ONLY : ab_min, ihe4, langue, m_ch, nchim,
	1 nom_elem, precix
	USE mod_kind
	USE mod_numerique, ONLY : bsp1dn, bvald, gauss_band, intgauss,
	1 linf, no_croiss
	USE mod_variables, ONLY : chim, dim_ch, knotc, mc, mct, n_ch
	...
	INTEGER, PARAMETER :: compt_max=10, err_lim=1.d0, err_max=0.1d0	
	INTEGER, ALLOCATABLE, DIMENSION(:) :: indpc, indpc0
	INTEGER, SAVE :: bloc
	INTEGER :: col, colonne, compt, i, id, ie,
	1 ig, ipe, iv, ive, j, k, l=1, ligne, nl, rang, spi, spl,
	2 sple
...
c on évite les points muultiples
...
c	 construction du vecteur temporaire des corrections
	 ...
	 ALLOCATE(temp(nchim,dim_ch)) ; temp=RESHAPE(b,SHAPE(temp))
	 ...
	 DO spl=1,dim_ch
	  B2: DO iv=1,nchim	  
	   IF(iv == 1 .OR. iv == ihe4)THEN	  
	    ...
	    IF(corr*ABS(temp(iv,spl)) > 0.6d0*ABS(chim(iv,spl)))
	1     corr=corr/2.d0
	   ENDIF
	  ENDDO B2	!iv
	 ENDDO		!spl
...
c	 nouvelle solution
	 ...
	 chim=chim-temp*corr ; DEALLOCATE(temp)
...
c	 écritures
	 ...
	 WRITE(*,100)compt,errg,nom_elem(1),nom_elem(ihe4),corr,err,
	1  nom_elem(ive),sple,ipe
...
c on accepte une mauvaise convergence pour un nouveau pas temporel
	 ...
	 ELSE
	  IF(errg < err_max)THEN
	   SELECT CASE(langue)
	   CASE('english')
	    PRINT*,'bad conv. in resout_chim, number of iterations :',
	1    compt	  
	   CASE DEFAULT
	    PRINT*,'conv. forcée dans resout_chim, nb. itérations :',
	1    compt
	   END SELECT
	   ok=.TRUE. ; EXIT B1
	  ...
	  ELSE  
	   SELECT CASE(langue)
	   CASE('english')
	    PRINT*,'no conv. in resout_chim, number of iterations :',
	1    compt	  
	   CASE DEFAULT
	    PRINT*,'pas de conv. dans resout_chim, nb. itérations :',
	1    compt
	   END SELECT
	   EXIT B1
	  ENDIF
