...
c************************************************************************
	...
	SUBROUTINE etat_opalX(pp,tt,xchim,deriv,
...
c	deriv=.TRUE. : évite le calcul de certaines dérivées
...
c 	REAL (kind=dp), PARAMETER :: Na=6.0221367d+23, k=1.380658d-16,
c	1 c=2.9979245d+10,sigmacc=1.8914785d-3, 

 	REAL (kind=dp), PARAMETER :: aprop=83.14510d0,
	1 sigmacc=1.8914785d-3 

c     Calling propram should contain something like following lines.
c     See subroutine esac for definitions
c      parameter (mx=6,mv=10,nr=169,nt=191)
c      iorder=9  gives all 1st and 2nd order data. See instructions
c                  in esac.
c      irad=0 does not add radiation  corrections
      
	INTEGER, PARAMETER :: iorder=9, irad=1, mx=6, mv=10, nr=169,
	1 nt=191	        
	...
	REAL (kind=dp) :: r, stor, stor0, dstor, x, t6, p12, p, t,
	1 duro
...
1	 FORMAT(/,'-------------Equation d''état-----------------',/,
	1 'Equation d''état OPALX si T dans [ 2010 , 1.e8 ], EFF sinon',/,
	2 'der. num.; (p,T)-->(ro,T) calculé par rhoofp',/) 
	ENDIF
      	
     	t6=t*1.d-6 ; p12=p*1.d-12 ; x=xchim(1)
      	
      	ro=rhoofp(x,t6,p12,irad,ok)  ! calculate density (ro) for fixed P.
      	IF(ok)THEN      	
	 CALL esac(x,t6,ro,iorder,irad,ok) ! calc EOS; use ro from rhoofp       	
	 IF(ok)THEN
...
c	   dérivées / P
	   ...
	   dstor=stor-stor0 ; p=stor ; p12=p*1.d-12
      	   r=rhoofp (x,t6,p12,irad,ok)  ! calculate density (r) for fixed P.
      	   IF(ok)THEN
       	    CALL esac(x,t6,r,iorder,irad,ok) ! calc EOS; use r from rhoofp
	    IF(ok)THEN
...
c*************************************************************************
	...
	SUBROUTINE esac (xh,t6,r,iorder,irad,ok)
...
c--------------------------------------------------------------
	...
	REAL (kind=dp) :: slr,slt,moles,xh,
	1 t6,r,dixr,p0,z,sum1,sum2,sum23,sum33,tmass,
	2 eground,fracz
...
c	Nécessairement : eos_opalX.bin
...
2000	FORMAT(8es10.3)
	...
	ENDIF
	chain=TRIM(nom_chemin)//'eos_opalX.bin'
...
c********************************************************************
	...
	REAL (kind=dp) FUNCTION rhoofp(x,t6,p,irad,conv)
	...
	REAL (kind=dp) :: rat,pr,t6,p,pnr,x,pmax,pmin,
...
c--------------------------------------------------------------
	...
	IF(itime /= 12345678)THEN
	 ...
	 CALL esac(xinit,tinit,rinit,1,0,conv) ; IF(.NOT.conv)RETURN
	ENDIF
...
15	  FORMAT('The requested pressure-temperature not in table',/,
	...
	CALL esac (x,t6,rhog1,1,0,conv) ; IF(.NOT.conv)RETURN
	p1=eos(1)
	IF(p1 > pnr)THEN
	...
	ELSE
	 ...
	 CALL esac(x,t6,rhog2,1,0,conv) ; IF(.NOT.conv)RETURN
	 p2=eos(1)
	ENDIF
...
1	CONTINUE
	...
	CALL esac(x,t6,rhog3,1,0,conv) ; IF(.NOT.conv)RETURN
	p3=eos(1)
