...
c-------------------------------------------------------------------------
	...
	USE mod_donnees, ONLY : Ipg, langue, m_qs, ne, ord_qs, pturb,
	1 r_qs
	USE mod_kind
	USE mod_numerique, ONLY: bsp1dn, bvald, coll, gauss_band, linf,
	1 no_croiss
	USE mod_variables, ONLY: bp, dim_qs, knot, m23, 
	1 n_qs, q, qt, r2, xl
	...
	INTEGER, SAVE :: lq=1, nbj, ncoll, nl, n_qsp
...
c	 points de collocation sur [i,i+1] peuvent être tabulés une
c	 fois pour toutes
...
c	 WRITE(*,2000)REAL(i),REAL(ip),xcoll(i),q(ip),fac(ip)
	 DO j=1,m_qs
	  cq=i+j-1
	  CALL bsp1dn(ne,bp,q,qt,n_qs,ord_qs,knot,.TRUE.,xcoll(cq),
	1   lq,f,dfdq)
...
c	PRINT*,qmax,vare ; PAUSE'qmax,vare,après correction'
...
c	pour lim_zc il faut les valeurs de r2 et m23 de la
c	dernière itération	
