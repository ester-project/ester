...
c----------------------------------------------------------------
     ...
     2 msol, m_ch, m_ptm, m_rot, m_tds, nchim, ne, nrot, pi, pturb,
     ...
     1 dxchim_t, xchim, xchim_t
      ...
      REAL (kind=dp), SAVE :: cte6, cte11, cte13, cte14, cte14t,
     ...
     1 eps0, gam, gam0, gam1, gamma1, grad, grad0 , gradrad, gravs2, hp,
     2 kap, mext0, mk, pext0, pgn, prn, psist, psist0,
     ...
     5 trn, t_t=0.d0, t_t0, u, u_t=0.d0, u_t0, u0, x00, v, w
      ...
      CHARACTER (len=7), SAVE, DIMENSION(8) :: variable=(/ 'ln Ptot',
     1 'ln T   ','R      ','L      ','M      ','Psi    ','ln rho ',
     2 'ln Pgaz'/)
...
c accélération centrifuge	 
	 acc_c=rsol*ay3*w**2 ; reprend=acc_c > gravs2
	 IF(reprend)THEN
	  SELECT CASE(langue)
	  CASE('english')
...
c      pour éviter cette disposition supprimer les "c" devant reprend
c      et RETURN

       IF(y(3,0)*y(5,0)*y(4,0) < 0.d0)THEN
        WRITE(*,"('static_r, r, l, ou m < 0, on tente de CV, cx=',i4)")cx
c       WRITE(*,2000)xcoll(cx),y(3,0),y(5,0),y(4,0)
c       PRINT*,y(1:ne,0)
c       PAUSE'erreur, static_r, m, r ou l < 0'
...
c      PRINT*,' '
       ...
       ENDIF
       CALL thermo(prn,pgn,trn,ay5,y(4,0),ay3,dlpp,xchim,dxchim,
     ...
     7 dhpr,dhpm,gradrad,alfa,beta,gamma1,radiatif)
...
1006	    FORMAT('Too large change of TdS at m/Mstar=',es10.3)
	   CASE DEFAULT
	    WRITE(*,6)ay5/mstar
6	    FORMAT('trop de varariation de TdS en m/Mstar=',es10.3)
	   END SELECT
...
c        PAUSE'avant bsp1dn1'
     ...
     1    knotc,.TRUE.,MIN(am23,mc(n_ch)),l,xchim,dxchim)
         ...
         CALL thermo(prn,pgn,trn,ay5,yd(4,0),ay3,dlpp,xchim,dxchim,
     1   ro,drop,drot,drox,u,dup,dut,dux,
     2   grad,dgradpt,dgradp,dgradt,dgradx,dgradm,dgradl,dgradr,dgradlpp,
     3   gam,dgampt,dgamp,dgamt,dgamx,dgamm,dgaml,dgamr,dgamlpp,     
     4   epsilon,depsp,depst,depsx,kap,dkapp,dkapt,dkapx,
...
c        la rotation
	 ...
	 CASE(0,1,2)
	  w=wrot ; dwdm=0.d0
	 CASE(3)
	  CALL bsp1dn(nrot,rota,mrot,mrot,n_rot,m_rot,knotr,.TRUE.,
	1   am23,l,frot,dfrot)
	  IF(no_croiss)PRINT*,'Pb. at 7 in static_r'
	  w=frot(1) ; dwdm=dfrot(1)*2.d0/3.d0/am13
	 END SELECT

         v=ro*ay3/prn*w**2 ; dyv=cte25*fac(ip)*v	!rotation

         teta=fac(ip)*(ro*(ay5/ay3**2/prn*(cte21+cte22*grad)
...
c        WRITE(*,2000)y(5,0),mext,be(1)
	...
	IF(no_croiss)PRINT*,'Pb. at 12 in static_r'
	xchim=ABS(xchim) ;  CALL chim_gram(xchim,dxchim)	
	CALL etat(EXP(y(1,0)),EXP(y(2,0)),ABS(xchim),.FALSE.,
	1 ro,drop,drot,drox,u,dup,dut,dux,
	2 delta,deltap,deltat,deltax,cp,dcpp,dcpt,dcpx,
	3 gradad,dgradadp,dgradadt,dgradadx,alfa,beta,gamma1)
	be(1)=y(7,0)-LOG(ro)
