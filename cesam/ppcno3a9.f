...
c**************************************************************************
	...
	SUBROUTINE ppcno3a9(t,ro,comp,dcomp,jac,deriv,fait,
	1 epsilon,et,ero,ex,hhe,be7e,b8e,n13e,o15e,f17e)

c	rouine private du module mod_nuc
...
c	H2, Li7, Be7 a l'équilibre

c	un premier appel a rq_reac initialise et definit le nb.
c	d'éléments chimiques pour lesquels les reac. nuc. sont tabulees
c	dans ppcno93a9 on ajoute Ex, soit nchim+1
c	on utilise une table ppcno+3a a 9 éléments cree par tab_reac
c	dont on doit indiquer le nom dans le fichier de donnees *.don
...
c	 ENDDO
	 ...
	 IF(.not.inversible)THEN
	  PRINT*,'ppcno3a9, matrice du calcul des abon. non inversible'
	  PRINT*,'ARRET' ; STOP
	 ENDIF
...
1002	  FORMAT(/,'PP + CNO + 3alpha thermonuclear reactions',/)
...
1020	  FORMAT(/,'Initial abundances/mass computed with :',/,
	...
	3 'Z0 = 1-X0-Y0 = C12+C13+N14+N15+O16+O17+Ex',/)	
	  WRITE(2,1001)ab_ini(1:nchim) ; WRITE(*,1001)ab_ini(1:nchim)
1001	  FORMAT('H1 :',es10.3,', He3 :',es10.3,', He4 :',es10.3,
...
1009	  FORMAT(/,'Initial abundances/number: 12+Log10(Ni/Nh)',/,
	...
	3 ', N15 :',es10.3,', O16 :',es10.3,', O17 :',es10.3,', Ex :',
	4 es10.3)
...
1005	  FORMAT(/,'threhold for neglectable abondunces/mass :',/,
	1 'H1:',es10.3,', He3:',es10.3,', He4:',es10.3,
	2 ', C12:',es10.3,', C13:',es10.3,/,'N14:',es10.3,
...
1008	  FORMAT('Temporal evolution, test of precision on H1 He4')
	 CASE DEFAULT
	  WRITE(2,2) ; WRITE(*,2) 
2	  FORMAT(/,'Réactions thermonucléaires, cycles PP+CNO+3alpha',/)
...
c	 équations d'evolution
	...
	2 +r(16)*comp(4)+r(17)*comp(8))*comp(3)			!He4
...
c	 PRINT*,'ppcno3a9, vérifications SUM dcomp*nucleo=0'
...
c	2 +(2.d0*r(3)*comp(2)-r(4)*comp(3))*comp(2)			!H1
...
c	2 +r(16)*comp(4)+r(17)*comp(8))*comp(3)			!He4
	...
	4  +drx(16,i)*comp(4)+drx(17,i)*comp(8))*comp(3)
	  ENDDO
	 
c	  équation dcomp(4)
c	  dcomp(4)=(-r(8)*comp(4)+r(12)*comp(7))*comp(1)
c	1 +(r(15)*comp(3)**2-r(16)*comp(4))*comp(3)	!C12

	  jac(4,1)=-r(8)*comp(4)+r(12)*comp(7)			!d /H1
	  jac(4,3)=3.*r(15)*comp(3)**2-r(16)*comp(4)		!d /He4	 
	  jac(4,4)=-r(8)*comp(1)-r(16)*comp(3)			!d /C12
...
c	1 +(r(16)*comp(4)-r(17)*comp(8))*comp(3)		!O16
...
c	  dcomp(10)=-SUM(dcomp*anuc)/anuc(10) 	conservation des baryons	
...
c        ecriture des taux de reaction				!YLD
c        write(67,*) (comp(i),i=1,nchim),t,ro,(r(i),i=1,14)	!YLD
...
c	 PRINT*,'h2,li7,be7' ; WRITE(*,2000)h2,li7,be7
	...
	5 +dqo(17)*comp(8))*comp(3)
	6 +(q(2)*dh2ro+q(5)*dli7ro+q(7)*dbe7ro)*comp(1)+q(6)*mue*dbe7ro	

	  ex(1)=2.*q(1)*comp(1)+q(2)*h2+q(5)*li7+q(7)*be7
	...
	CASE DEFAULT
	 PRINT*,'ppcno3a9, fait ne peut valoir que 1, 2, 3 ou 4'
