...
c d'où la variation de masse totale et la nouvelle masse totale mstar
c calcule la masse qui correspondait au temps t pour la masse
c en chaque point
...
c----------------------------------------------------------------
	...
	1 nchim, mdot, m_ch, m_ptm, m_qs, nchim, ne, nom_pertm, pturb,
	2 ord_qs, secon6, t_inf
	USE mod_etat, ONLY : etat
	USE mod_kind	
	USE mod_nuc, ONLY : l_planet, nuc, planetoides
	USE mod_numerique, ONLY : bsp1dn, no_croiss, sum_n
	USE mod_variables, ONLY : bp, chim, chim_gram, knot, knotc,
	1 knot_ptm, mc, mct, mstar, mstar_t, n_ch, n_ptm, n_qs,
	2 old_ptm, q, qt, x_ptm, xt_ptm
	...
	REAL (kind=dp), DIMENSION(1,n_qs) :: dm
	REAL (kind=dp), DIMENSION(ne) :: df, f
	REAL (kind=dp), DIMENSION(nchim) :: dxchim, depsx, xchim, xchimm
	REAL (kind=dp), DIMENSION(n_qs+6) :: mt
	REAL (kind=dp), DIMENSION(n_qs) :: m, vt1, vt2
	REAL (kind=dp), SAVE, DIMENSION(5) :: epsilon
	
	REAL (kind=dp), SAVE :: cte1, ehh, ebe7, eb8, ef17, en13, eo15, mev
	REAL (kind=dp) :: adm, alfa, beta, be7,  bid,b8, cte2, cp, dcpp, dcpt,
	1 dcpx, def, delta, deltap, deltat, deltax, depsro, depst, drop, drot,
	2 drox, dup, dut, dux, gamma1,gradad, dgradadp, dgradadt, dgradadx,
	3 hh, m_planet,  n13, o15, f17,p, ro, t, u
...
2000	FORMAT(8es10.3)
	...
	IF(init)THEN
	 ...
	 mev=1.d6*eve	!énergie des neutrinos selon Clayton p.380 et 392
