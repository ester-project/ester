...
! ***********************************************************************
      ...
      integer function alloc_kap_handle_using_inlist(inlist,ierr) result(handle)
         ...
         integer, intent(out) :: ierr ! 0 means AOK.
         ierr = 0
         print*,"alloc_kap_handle_using_inlist 1 "
         
         if (.not. kap_is_initialized) then
            ierr=-1
            return
         endif
         print*,"alloc_kap_handle_using_inlist 2 "
         
         handle = do_alloc_kap(ierr)
         print*,"alloc_kap_handle_using_inlist 3 ", trim(inlist)
         if (ierr /= 0) return
         call read_namelist(handle, inlist, ierr)
         print*,"alloc_kap_handle_using_inlist 4 "
         if (ierr /= 0) return
         print*,"alloc_kap_handle_using_inlist 5 "
         call kap_setup_tables(handle, ierr)
         call kap_setup_hooks(handle, ierr)
         print*,"alloc_kap_handle_using_inlist 6 "
...
!             print*,"i=",i,"iz=",iz,"chem_id(i)=",chem_id(i)
!             print*,"i=",i,"iz=",iz
            select case(iz)
            case (6)
               xc = xc + xa(i)
...
!                        print*,"2 kap=",kap
!                        print*,"2 dlnkap_dlnT=",dlnkap_dlnT
