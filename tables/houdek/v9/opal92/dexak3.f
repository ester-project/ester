...
c        (tlg>6., ....)
c        use only last 5 points (5 tlg-values) for
c        extrapolation
c
c  History:
c
...
c
      open(29,file='OPINTPATH',form='formatted',status='old',err=9030)
      read(29,'(A)')optabe
      read(29,'(A)')paderi
      read(29,'(A)')ivadat
      close(29)
...
c          write(77,'(7X,17f7.4)') (dlg(i,7,l),i=1,nxi)
c
c         first extrapolate for 7.4<=tlg<=8.0 (46<=j<=50)
c                               0.0<=rlg<=1.0 (15<=i<=17)
c
          do 2001 j=46,nyi 
            if(ival(j,itab,l).lt.nxi) then
              nd=ival(j,itab,l)
...
c
c         now do interpolation for rlg={-0.5,0.0}...0.5 (i={14,15}...16)
c                           4.5xx<=tlg<=7.3             (j=16...45)
c                     
c
          do 4001 j=16,45
            if(ival(j,itab,l).lt.nxi) then
c             nd should be 13 or (14 for j>=33)
              nd=ival(j,itab,l)
              do 4005 i=1,nd
                 yd(i) = dlg(i,itab,l)
                 zd(i) = opa(i,j,itab,l)
...
9010  print *,'dopextra: error in opening of file ',ivadat
      stop
9030  print *,'dopinit: error in opening of OPINTPATH'
      print *,'dopinit: try to assign a logical name to OPINTPATH'
      print *,'dopinit: with setenv OPINTPATH <absolute_path_name> in '
      print *,'dopinit: a csh where <absolute_path_name> is the file '
      print *,'dopinit: containing the absolute pathnames of the '
      print *,'dopinit: input files optabe.bin, pderivs.dat, ival.dat'
      stop
c
      end
