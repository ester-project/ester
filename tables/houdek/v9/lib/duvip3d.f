      SUBROUTINE  UVIP3D(NP,ND,XD,YD,NI,XI, YI,DI)
...
C   DI = array od dimension NI, where the derivatives of the
C        desired points are to be stored.
C
...
C - With a third-degree polynomial.  --  cf. Equation (3)
            ...
            DI(II)=A1+XX*(2.d0*A2+XX*3.d0*A3)
          ELSE
