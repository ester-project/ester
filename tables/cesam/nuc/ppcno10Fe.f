...
c**************************************************************************
	...
	SUBROUTINE ppcno10Fe(t,ro,comp,dcomp,jac,deriv,fait,
...
c	H2 et Be7 a l'équilibre, Ex est l'élément moyen de complément
c	Fe56 et Ex n'intéressent que la diffusion

c	un premier appel a rq_reac initialisé et définit le nb.
c	d'éléments chimiques pour lesquels les reac. nuc. sont tabulées
c	dans ppcno10Fe on ajoute Ex, soit nchim+1, puis
...
c----------------------------------------------------------------------
	...
	1 dbe7mue, dbe7h, charge_ex , mass_ex , sum_a
...
c on ajoute Fe56
	 ...
	 nucleo(nchim-1)=m(26)	!nucleo de Fe
	 zi(nchim-1)=c(26)	!charge de Fe
	 WRITE(text,10)NINT(m(26))
	 nom_elem(nchim-1)=elem(NINT(c(26)))//text !nom de Fe
	 	 
c Ex : élément fictif moyenne des éléments # Li, CNO, Fe

	 charge_ex=0.d0 ; mass_ex=0.d0 ; sum_a=0.d0
	 B1: DO i=4,nelem_ini		!à partir de Li=3
...
c	 PRINT*,nchim ; WRITE(*,2000)nucleo(1:nchim) 
	 ...
	 DO j=4,nchim
	  ...
	  a(12,j)=-abon_rela(26)	!somme comp(i) Fe
	 ENDDO
...
c	 ENDDO
	 ...
	 IF(.not.inversible)THEN
	  PRINT*,'ppcno10Fe, matrice calcul abondances non inversible'
	  PRINT*,'ARRET'
	  STOP
	 ENDIF	
...
1020	  FORMAT(/,'Initial abundances/mass computed with :',/,
	...
	3 'Z0 = 1-X0-Y0 = Li7+C12+C13+N14+N15+O16+O17+Fe56+Ex',/)	
...
1	  FORMAT('H1 :',es10.3,', He3 :',es10.3,', He4 :',es10.3,
	...
	3 ', O17 :',es10.3,', Fe56 :',es10.3,', Ex :',es10.3)
...
c	 dcomp(11)=0.d0 ; dcomp(12)=0.d0

c	   Pour vérifications SUM dcomp*nucleo=0

c	 PRINT*,'ppcno10Fe, vérifications SUM dcomp*nucleo=0'
c	 WRITE(*,2000)DOT_PRODUCT(dcomp,anuc) ; PAUSE'vérif' 
	 
c conservation des baryons	 

	 dcomp(12)=-DOT_PRODUCT(anuc,dcomp)/anuc(12)		!Ex
...
c	3 +r(14)*comp(10))*comp(1)+2.d0*r(3)*comp(2)**2		!H1
	...
	3 -r(7)*comp(1)*dbe7h					!d /H1
	  ...
	  DO i=1,nchim		!dépendances effet d'ecran et be7/muex
	   jac(1,i)=jac(1,i)
...
c	1 +r(4)*comp(3))*comp(2)				!He3
	  ...
	  DO i=1,nchim		!dépendances dues a l'effet d'ecran
	   jac(2,i)=jac(2,i)
...
c conservation des baryons	 
c	 dcomp(nchim)=-SUM(anuc*dcomp)/anuc(nchim)
