...
! les données de base sont celles de ctes_94
! les corrections nécessaires sont apportées pour les autres ensembles

! la routine print_ctes.f permet de les écrire
...
! origine des principales données	
	source='Handbook chem. phys. 94 + NACRE'
	
! constantes de base début: ctes_94 c'est le défaut----------------------
...
! la gravité dépend de la valeur retenue pour la masse solaire	
	g=gmsol/msol

! masses des isotopes en unités atomiques	
...
! constantes de Toulouse (département de haute Garonne 31)
	CASE ('ctes_31')
