...
C       variables needed for relativistic corrections
      real*8 con_nr1_ratio, con_d1_ratio,
     &  beta, fermi_dirac_ct,
     &  fd32(6), fd12(5),
     &  fex1, fex1f, fex1t,
...
C      variables needed for EFF-style summations and derivatives.
      integer*4 mordermax, maxc, maxkind,
     &  ikind, ioffset, nderiv
      parameter (mordermax = 9)
      parameter (maxc = (mordermax+1)*(mordermax+1))
      parameter (maxkind = 6)
      parameter (nderiv = 9)
      real*8 ccoeff(maxc, maxkind)
      integer*4 mforder(maxkind), mgorder(maxkind), n_max
      real*8 sum(1+nderiv)
      real*8
     &  fexi, fexif, fexit,
     ...
     &  fexl_psif2, fexl_psift, fexl_psit2
      real*8 wf, vf, uf, duf, duf2, g, vg, ug, fdf, dug, dug2,
     &  power
      real*8 acon, bcon, ccon, dcon, econ
      integer*4 index_exchange
