...
C       morder_in = -23 is same as 23 in non-relativistic limit.
      implicit none
      integer*4 mstar, morder_in, morder, norder
      real*8 rhostar(mstar+6),pstar(mstar+6),sstar(mstar), ustar(mstar),
     &  fl, tcl, rhostarcon, pstarcon
      integer*4 mderiv, mordermax
      parameter (mderiv = 2,mordermax = 8)
      integer*4 maxfd_direct
...
C         morder = 8 call to fermi_dirac_coeff.
      real*8 ccoeff(3*(mordermax+1)*(mordermax+1)),
     &  sum(mderiv+1+7),
     &  fd(0:4), fermi_dirac_ct,
     &  fd_direct(maxfd_direct,3)
      integer*4 morder_old, i, ideriv, nderiv, ioffset, moffset
      real*8 f, wf, tc, g, vf, vg, uf, duf, duf2, ug, dug, dug2,
