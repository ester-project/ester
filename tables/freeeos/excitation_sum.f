...
C      specific neff, ell, and weight data for helium 1 (supplied by Dappen)
      ...
      integer*4 ifexcited, ifsame_zero_abundances,
...
C      number of xextrasum variables
      ...
      real*8 tl, bmin(izhi), bion(nions+2),
     ...
     &  xextrasumt(nxextrasum), xextrasum_dv(nions+2,nxextrasum)
      ...
      integer*4 iz, izqstar, index, max_index,
     &  ielement, ion_start, ion, nmin_s, ifpi_fit
      integer*4 n, ix, min_nmin_max,
...
C      4 is indistinguishable from 10 for solar comparisons.
      ...
      real*8 exparg, exparg_lim, exp_lim, ground_state_lim,
...
C      and onepqs = onepq*qratio_scale
      real*8 exparg_shift, qratio_scale, qratio_us, onepq, onepqs
      parameter(exparg_shift = 600.d0)
      real*8 ln_ground_occ
      integer*4 index_x(nx)
...
C      add components to each excitation sum and its derivatives.
      implicit none
C      input variables:
      integer*4 nxextrasum, nions, max_dvindex
      integer*4 ion_start1, ion_end1, ion_start2, ion_end2,
     &  inv_ion(nions+2)
      logical ifmhd_logical, iz_logical, ifnr03, ifnr13,
     &  ifsimple_index
      real*8 nuvar, nuvarf, nuvart, nuvar_dv(max_dvindex),
...
C      output variables:
     ...
     &  xextrasumt(nxextrasum), xextrasum_dv(nions+2,nxextrasum)
C      local variables:
      integer*4 ion_index, ion_index1, inv_ion_index 
      real*8
