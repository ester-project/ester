...
\begin{equation} \Delta\phi = 4\pi G\rho\label{poiss}\end{equation}
...
in the convective core. This equation gives the temperature gradient as
a function of the pressure gradient thanks to the thermodynamics relation
...
\subsection{Brunt-V\"ais\"al\"a frequency}

In a spherically symmetric model, thus in 1D, the \BVF\ reads

\begin{equation}
 N^2 = g\lp\frac{1}{\Gamma_1}\dnr{\ln P}-\dnr{\ln\rho}\rp
\label{bv}
\end{equation}
Such an expression can be rewritten using the entropy gradient, namely

\begin{equation}
N^2 = -\frac{\delta}{c_p} \vg\cdot\na S
\label{bvf}
\end{equation}
Eq. (\ref{bvf}) can be easily generalized to a 2D case. It is sufficient
to replace gravity by the effective gravity, namely gravity plus the
centrifugal acceleration. Hence, for the 2D case we have

\begin{equation}
N^2 = -\frac{\delta}{c_p} \vg_{\rm eff}\cdot\na S 
\label{bvfe}
\end{equation}
where effective gravity $\vg_{\rm eff}$ is given by

\begin{equation}
\vg_{\rm eff} = -\nabla\phi+s\Omega^2\es\; .
\label{geff}
\end{equation}
In (\ref{geff}) the gravitational potential $\phi$ is derived from the Poisson
equation (\ref{poiss}).
