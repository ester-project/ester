...
\hline
\end{tabular}
    \caption{Example of scaled mass fractions of metals for (approximately) the chemical
...
The default EOS is the OPAL one but with the possible use of arbitrary chemical
composition this equation of state is too restrictive. We therefore also
implemented the FreeEOS library
({\tt https://freeeos.sourceforge.net/documentation.html}). For each point
in the stellar grid, ESTER builds a 20-element composition vector and
calls the FreeEOS routine to obtain density, pressure, entropy, adiabatic
gradients, and other derivatives required for the structure solver. The 20
elements included are H, He, C, N, O, Ne, Na, Mg, Al, Si, P, S, Cl, Ar,
Ca, Ti, Cr, Mn, Fe, and Ni, with their isotopic mixtures determined from
the user-defined composition. This EOS option can be used in conjunction
with either the default OPAL opacity tables or the monochromatic opacity
tables (Section 2.2.2); when used with the latter, ESTER ensures that
the EOS and opacity calculations remain compositionally consistent.
This latter point is because the elements in OP monochromatic opacities
are slightly less: they are H, He, C, N, O, Ne, Na, Mg, Al, Si, S, Ar,
Ca, Cr, Mn, Fe and Ni, so 17 elements, while freeEOS includes 3 others,
namely P, Cl and Ti. Thus, for consistency these latter three elements
are not used either in eos and opacities.
...
\begin{itemize}
\item {\tt simple\_CN}. Simplified formulation of pp and CNO cycles.
Here the abundance of X$_{\rm CNO}$ is given by $(C+N)*Z$.
\item {\tt simple\_ON}. Simplified formulation of pp and CNO cycles. 
Here the abundance of X$_{\rm CNO}$ is given by $(O+N)*Z$. Since Carbon
and Oxygen abundance (relative to metallicity Z) differ, the energy
production differs, and so the luminosity.
...
ESTER native format is basic binary format. This is a bit outdated since hdf5
(see below) offers a more flexible storage. However, remarks below are valid
also for hdf5 files.
...
plt.show()              # Needed in non-interactive mode of matplotlib
#---------------------------------------------------------------------
\end{minted}
...
However, a more efficient way of accessing the data is to use the ester module
as when using ESTER native format, namely
...
A=star1d("model_file.h5")  # Loads a model stored in hdf5 format
B=star2d("model_file.h5")  # if you use a 2D-model

print(A.p[0,0])         # Prints the central pressure
print(A.M)              # Prints the mass

plt.plot(A.r[:],np.log10(A.rho[:])) # plot the radial profile of rho
