...
the Jacobian of the transformation \eq{themap} is non-singular, namely
...
\frac{\partial V^i}{\partial x^i}+\frac{\partial\log\sqrt{|g|}}{\partial
...
ones.
\end{itemize}
\subsection{The meridional streamfunction}

In a steady state the velocity field verify $\na\cdot\rho\vv=0$. Setting
$\vW=\rho\vv$ the field $\vW$ verifies

\begin{equation}
\partial_\zeta(\sqrt{|g|}W^\zeta) +
\partial_\theta(\sqrt{|g|}W^\theta)=0
\end{equation}
from which we introduce the streamfunction $\psi$ such that

\begin{equation}
W^\zeta = \frac{1}{\sqrt{|g|}}\partial_\theta\psi \andet W^\theta =
-\frac{1}{\sqrt{|g|}}\partial_\zeta \psi
\end{equation}
The lines $\psi=\cst$ are streamlines. In \ester the meridional velocity
field is described with the function $G$ which is related to the stream
function by

\[ \psi = r\sth G\]
hence we get

\[ \rho v_r = \frac{1}{r\sth}\partial_\theta(\sth G) -
\frac{r_\theta}{rr_\zeta}\partial_\zeta G\]
and

\[ \rho v_\theta = -\frac{1}{rr_\zeta}\partial_\zeta(rG)\]
...
\subsection{Meridional stream function}
...
\beq \dzeta{\sqrt{g}W^\zeta} + \dtheta{\sqrt{g}W^\theta} = 0 \with
\sqrt{g} = r^2r_\zeta\sth\eeq
so we choose for the meridional stream function $\psi$ of the mass flux $\vW$

\beq \dzeta{\psi} = \sqrt{g}W^\theta \andet \dtheta{\psi} =
-\sqrt{g}W^\zeta\eeqn{stream}
...
w_r & dr \\
w_\theta & rd\theta \end{array}\right|=0\quad \ssi \quad \vW \;{\rm parallel\;
to\;} \dl
\eeqn{determ}
...
\beq w_r =
-\frac{1}{r^2\sth}\lp\dtheta{\psi}-\frac{r_\theta}{r_\zeta}\dzeta{\psi}\rp
\andet w_\theta = \frac{1}{rr_\zeta\sth}\dzeta{\psi} \eeqn{w_sph}
We therefore should prove that

\[ d\psi = 0 \impl w_\theta dr - w_rrd\theta = 0\]
or that

\beq \frac{1}{rr_\zeta\sth}\dzeta{\psi}dr
+\frac{1}{r^2\sth}\lp\dtheta{\psi}-\frac{r_\theta}{r_\zeta}\dzeta{\psi}\rp
rd\theta = 0 \eeqn{dpsi}
...
\[ d\psi= 0 \impl r\sth dG  +\sth Gdr + \cth Gd\theta =0\]
...
which is the same as \eq{stream} up to the sign.
...
\beq \sqrt{g}W^\zeta = \sth\sum_\ell u^\ell(\zeta)\YLz\eeq
\beq \sqrt{g}W^\theta = \sth\sum_\ell v^\ell(\zeta)\partial_\theta\YLz\eeq
...
\beq  \psi^\ell(\zeta) = \frac{u^\ell(\zeta)}{\ell(\ell+1)}
\eeq
So we just need to compute the $u^\ell(\zeta)$ on the radial grid. We
recall that

\beq W^\zeta = \frac{\rho}{r_\zeta}\lp v_r - \frac{r_\theta}{r}v_\theta\rp
\eeq
so that

\beq u^\ell\YLz=\rho r^2\lp v_r - \frac{r_\theta}{r}v_\theta\rp\eeq
